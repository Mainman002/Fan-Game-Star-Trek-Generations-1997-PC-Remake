...
func _ready() -> void:
	...
	add_command("list_all", commands_list, 0, 0, "Lists all commands and their descriptions.")
	add_command("scenes", levels, 0, 0, "Lists all levels.")
	add_command("levels", levels, 0, 0, "Lists all levels.")
	add_command("zones", levels, 0, 0, "Lists all levels.")
	add_command("list_scenes", levels, 0, 0, "Lists all levels.")
	add_command("list_levels", levels, 0, 0, "Lists all levels.")
	add_command("list_zones", levels, 0, 0, "Lists all levels.")
	...
	add_command("tp", change_scene, 1, 1, "Teleport to a level.")
...
func _input(event : InputEvent) -> void:
	...
	elif Input.is_action_just_pressed("console_font_plus"):
		if font_size <= 0:
			font_size = 16 # Use default font size of 16
		font_size = min(128, font_size + 2) # Limit to max of 128
		_update_font_size()
		get_tree().get_root().set_input_as_handled()
	elif Input.is_action_just_pressed("console_font_minus"):
		if font_size <= 0: font_size = 16 # Use default font size of 16
		font_size = max(8, font_size - 2) # Limit to minimum of 8
		_update_font_size()
		get_tree().get_root().set_input_as_handled()
	elif Input.is_action_just_pressed("console_autocomplete"):
		autocomplete()
		get_tree().get_root().set_input_as_handled()
	...
	if (suggesting):
		...
		suggesting = true
			...
			#var split_text := parse_line_input(line_edit.text)
				...
				#var command:String = split_text[0]
				#var param_input:String = split_text[1]
		...
		if (" " in line_edit.text): # We're searching for a parameter to autocomplete
			...
			if (split_text.size() > 1):
				...
				if command == "tp":
					# Autocomplete using scene_list
					for scene_name in scene_list:
						if !param_input or scene_name.begins_with(param_input):
							suggestions.append(str(command, " ", scene_name))
				elif command_parameters.has(command):
					for param in command_parameters[command]:
						if param.begins_with(param_input): # switched to begins_with
							suggestions.append(str(command, " ", param))
		else:
			...
			## Search all letters for possible results
			...
			var prev_index:int = 0
			for command in sorted_commands:
				if (!line_edit.text or command.begins_with(line_edit.text)):
...
func change_scene(scene : Variant, print_godot := false) -> void:
	var scene_manager:Node = null
	var _scene:String = str(scene).to_lower()

	if get_tree().root.has_node("Scene_Manager"):
		scene_manager = get_tree().root.get_node("Scene_Manager")

	if scene_manager and _scene in scene_list:
		toggle_console()
		#get_tree().paused = false
		scene_manager._change_scene(_scene)
		print_line("	   [color=light_blue]INFO:[/color] Teleporting to: %s" % _scene, print_godot)
	else:
		print_line("	   [color=gold]WARNING:[/color] Scene: %s, not in scene list" % _scene, print_godot)
...
func levels() -> void:
	var levels := []
	for level in scene_list:
		levels.append(level)  # level is already the string
	#levels.sort()
		rich_label.append_text("    ")
		rich_label.append_text(str(level, '\n'))
