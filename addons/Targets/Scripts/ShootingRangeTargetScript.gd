...
@onready var damage_number_script:DamageNumberScript = DamageNumberScript.new()
...
func hitscanHit(damageVal : float, _hitscanDir : Vector3, _hitscanPos : Vector3):
	...
	if !isDisabled and canDisplayDamageNumber:
		damage_number_script.displayNumber(damageVal, damNumSpawnPoint.global_position, 110, DamageNumberScript.DamageType.NORMAL)
